...
function Get-ParametersDefaultFirst {
	...
	BEGIN {
		...
		'OutVariable', 'PipelineVariable', 'Verbose', 'WarningAction', 'WarningVariable', 'Whatif', 'Confirm'
	...
	PROCESS {
		if ($defaultPSetName = $Command.DefaultParameterSet) {
			$defaultParameters = ($Command.ParameterSets | Where-Object Name -eq $defaultPSetName).parameters |
				Where-Object Name -NotIn $Common

			$otherParameters = ($Command.ParameterSets | Where-Object Name -ne $defaultPSetName).parameters |
				Where-Object Name -NotIn $Common
			...
			$parameters = $Command.ParameterSets.Parameters | Where-Object Name -NotIn $Common |
...
$Module = Get-Module -ListAvailable $ModuleManifestPath
	...
	Describe "Test help for $commandName" {
		...
		Context "Test parameter help for $commandName" {
			...
			$HelpParameterNames = $Help.Parameters.Parameter.Name |
				Where-Object { $_ -notin $ShouldProcessParameters } | Sort-Object -Unique
